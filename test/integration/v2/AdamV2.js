...
const { ethers } = hre;
const { expect } = chai;
const { createAdamV2, createTokens } = require('../../utils/createContract');
...
describe('Integration - AdamV2.sol - test/integration/v2/AdamV2.js', function () {
  let creator;
  let token;
  let feedRegistry;
  let adam;
  ...
  function createDao () {
    return adam.createDao(...paramsStruct.getCreateDaoParamsV2({ name: 'A Company' }));
  };
  ...
  describe('when createDao() called', function () {
    ...
    it('produces upgradable dao', async function () {
      ...
      const dao = await ethers.getContractAt('DaoV2', daoAddr);
