...
describe('DepositRewardBudgetApproval.sol - test/unit/v2/DepositRewardBudgetApproval.js', async () => {
    let creator;
    let executor;
    let referee;
    let referrer;
    ...
    let DepositRewardBudgetApproval;
    let ERC1967Proxy;
    let depositRewardBAImpl;
    let team;
    ...
    function encodeTxData(receiver, asset, amount, referrer) {
        return abiCoder.encode(
            ...
            [receiver, asset, amount, referrer],
        );
    }
    ...
    beforeEach(async () => {
        ...
        DepositRewardBudgetApproval = await ethers.getContractFactory(
    ...
    describe('initialize()', async () => {
        it('init with params with the least setting successfully', async () => {
            const contract = await ERC1967Proxy.deploy(
                ...
                DepositRewardBudgetApproval.interface.encodeFunctionData(
                    'initialize',
                    initializeParser({
            ...
            const depositRewardBA = await ethers.getContractAt(
                'DepositRewardBudgetApproval',
                contract.address,
            );

            expect(await depositRewardBA.name()).to.be.eq(
                'Deposit Reward Budget Approval',
            );
            expect(await depositRewardBA.token()).to.be.eq(rewardToken.address);
            expect(await depositRewardBA.liquidPool()).to.be.eq(
                liquidPool.address,
            );
            expect(await depositRewardBA.allowAnyAmount()).to.be.eq(true);
            expect(await depositRewardBA.totalAmount()).to.be.eq(
                ethers.BigNumber.from('0'),
            );
            expect(await depositRewardBA.referrerRewardAmount()).to.be.eq(
                ethers.BigNumber.from('0'),
            );
            expect(await depositRewardBA.refereeRewardAmount()).to.be.eq(
                ethers.BigNumber.from('0'),
            );
            expect(await depositRewardBA.text()).to.be.eq('text');
            expect(await depositRewardBA.transactionType()).to.be.eq(
                'transactionType',
            );
        });
    ...
    describe('executeParams()', async () => {
        ...
        beforeEach(async () => {
            ...
            depositRewardBA = await ethers.getContractAt(
                'DepositRewardBudgetApproval',
                contract.address,
            );
        });
        it('describes execute params', async () => {
            expect(await depositRewardBA.executeParams()).to.be.deep.equal([
                'address receiver',
                'address asset',
                'uint256 amount',
                'address referrer',
    ...
    describe('execute()', async () => {
        ...
        it('issues depositReward if depositReward > 0', async () => {
            const contract = await ERC1967Proxy.deploy(
                depositRewardBAImpl.address,
                DepositRewardBudgetApproval.interface.encodeFunctionData(
                    'initialize',
        ...
        it('issues refereeReward if refereeReward > 0', async () => {
            const contract = await ERC1967Proxy.deploy(
                depositRewardBAImpl.address,
                DepositRewardBudgetApproval.interface.encodeFunctionData(
                    'initialize',
            ...
            expect(await depositRewardBA.totalAmount()).to.deep.eq(
                BigNumber.from('96'),
            );
        ...
        it('issues both reward if > 0', async () => {
            const contract = await ERC1967Proxy.deploy(
                depositRewardBAImpl.address,
                DepositRewardBudgetApproval.interface.encodeFunctionData(
                    'initialize',
        ...
        it('stop issues if supply not enough', async () => {
            const contract = await ERC1967Proxy.deploy(
                depositRewardBAImpl.address,
                DepositRewardBudgetApproval.interface.encodeFunctionData(
                    'initialize',
        ...
        it('deposits Tokens to LP', async () => {
            const contract = await ERC1967Proxy.deploy(
                depositRewardBAImpl.address,
                DepositRewardBudgetApproval.interface.encodeFunctionData(
                    'initialize',
        ...
        it('deposits ETH to LP', async () => {
            ...
            liquidPool.deposit.should.be.calledWithValue(
                BigNumber.from('1000'),
            );
        ...
        it('blocks members if joined before', async () => {
            const contract = await ERC1967Proxy.deploy(
                depositRewardBAImpl.address,
                DepositRewardBudgetApproval.interface.encodeFunctionData(
                    'initialize',
            ...
            ).to.be.revertedWithCustomError(depositRewardBA, 'NotQualify');
        });

        it('blocks members if quit before', async () => {
            const contract = await ERC1967Proxy.deploy(
                depositRewardBAImpl.address,
                DepositRewardBudgetApproval.interface.encodeFunctionData(
                    'initialize',
