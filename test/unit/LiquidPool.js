...
describe('LiquidPool.sol', function () {
  ...
  describe('deposit()', function () {
    ...
    it('can deposit if meet minDeposit amount & minTokenToJoin', async function () {
      await dao.setMinTokenToAdmission(parseEther('1'));
    ...
    it('can not deposit if not meet minMemberTokenToJoin amount', async function () {
      ...
      await expect(lpAsSigner1.deposit({ value: parseEther('1') })).to.be.revertedWith('member token not enough');
    });
  });
  ...
  describe('assetEthPrice()', function () {
    it('return price: 0.0046 if feedRegistry return 0.0046', async function () {
      expect(await lp.assetEthPrice(token.address, parseEther('1'))).to.eq(parseEther('0.0046'));
      expect(await lp.assetEthPrice(token.address, parseEther('1000'))).to.eq(parseEther('4.6'));
      expect(await lp.assetEthPrice(token.address, parseEther('0.0001'))).to.eq(parseEther('0.00000046'));
    });
    it('return price: 0 if feedRegistry return 0', async function () {
      await feedRegistry.setPrice(parseEther('0'));
      expect(await lp.assetEthPrice(token.address, parseEther('1'))).to.eq(parseEther('0'));
    });
  ...
  describe('ethShares()', function () {
    it('return token amount by ratio of dp balance / totalSupply (tokenA: 100, supply: 100)', async function () {
      await lpAsSigner1.deposit({ value: parseEther('100') });

      expect(await lp.ethShares(parseEther('0.00001'))).to.eq(parseEther('0.00001'));
      expect(await lp.ethShares(parseEther('1'))).to.eq(parseEther('1'));
      expect(await lp.ethShares(parseEther('2'))).to.eq(parseEther('2'));
      expect(await lp.ethShares(parseEther('10'))).to.eq(parseEther('10'));
      expect(await lp.ethShares(parseEther('100'))).to.eq(parseEther('100'));
    });
    it('return token amount by ratio of dp balance / totalSupply (eth: 120, supply: 100)', async function () {
      ...
      expect(await lp.ethShares(parseEther('0.00001'))).to.eq(parseEther('0.000012'));
      expect(await lp.ethShares(parseEther('1'))).to.eq(parseEther('1.2'));
      expect(await lp.ethShares(parseEther('2'))).to.eq(parseEther('2.4'));
      expect(await lp.ethShares(parseEther('10'))).to.eq(parseEther('12'));
      expect(await lp.ethShares(parseEther('100'))).to.eq(parseEther('120'));
    });
  });
});
