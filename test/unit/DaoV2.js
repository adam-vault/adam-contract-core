...
describe('DaoV2.sol - test/unit/DaoV2.js', function () {
  ...
  beforeEach(async function () {
    ...
    mockAdam = await smock.fake('AdamV2');
    ...
    const Dao = await ethers.getContractFactory('DaoV2', { signer: adamAsSigner });
    const implDao = await Dao.deploy();
    const proxyDao = await ERC1967Proxy.deploy(implDao.address, '0x');

    dao = await ethers.getContractAt('DaoV2', proxyDao.address, adamAsSigner);
