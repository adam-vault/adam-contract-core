...
describe('EthereumChainlinkPriceGateway', async () => {
  ...
  beforeEach(async () => {
    ...
    EthereumChainlinkPriceGateway = await ethers.getContractFactory('EthereumChainlinkPriceGateway', { signer: creator });
    ...
    await feedRegistry.setPrice(tokenC.address, ADDRESS_ETH, parseEther('-1'));
  ...
  describe('isSupportedPair()', function () {
    ...
    it('returns fail if base is not support', async function () {
      expect(await priceGateway.isSupportedPair(unknown.address, ADDRESS_ETH)).to.eq(false);
    });
  ...
  describe('assetPrice(): base currency = ETH', function () {
    ...
    it('asset = 1tokenA, should return 0.25 base currency', async function () {
      expect(await priceGateway.assetPrice(tokenA.address, ADDRESS_ETH, parseEther('1'))).to.eq(parseEther('0.25'));
    });
    it('asset = 1tokenB, should return 0.5 base currency', async function () {
      expect(await priceGateway.assetPrice(tokenB.address, ADDRESS_ETH, parseUnits('1', 6))).to.eq(parseEther('0.5'));
    });
  });
  ...
  describe('assetEthPrice()', function () {
    ...
    it('asset = 1tokenA, return 0.25', async function () {
      expect(await priceGateway.assetEthPrice(tokenA.address, parseEther('1'))).to.eq(parseEther('0.25'));
    });

    it('asset = 1tokenB, return 0.5', async function () {
      expect(await priceGateway.assetEthPrice(tokenB.address, parseUnits('1', 6))).to.eq(parseEther('0.5'));
    });
  });
  ...
  describe('ethAssetPrice()', function () {
    ...
    it('give 1ETH, return 4tokenA', async function () {
      expect(await priceGateway.ethAssetPrice(tokenA.address, parseEther('1'))).to.eq(parseEther('4'));
    });

    it('give 1ETH, return 2tokenB', async function () {
      expect(await priceGateway.ethAssetPrice(tokenB.address, parseEther('1'))).to.eq(parseUnits('2', 6));
    });
  });
