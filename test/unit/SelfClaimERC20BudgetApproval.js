...
describe('SelfClaimERC20BudgetApproval.sol - test/unit/SelfClaimERC20BudgetApproval.js', async () => {
    let selfERC20BAImplementation;
    let budgetApproval;
    let team;
    let executor;
    let approver;
    ...
    let tokenA;
    let executee;
    let SelfClaimERC20BudgetApproval;

    beforeEach(async () => {
        [executor, approver, receiver, validator] = await ethers.getSigners();

        ({ tokenA } = await createTokens());
        SelfClaimERC20BudgetApproval = await ethers.getContractFactory(
    ...
    describe('Create Budget Approval', async () => {
        it('creates liquid ERC20 Budget Approval', async () => {
            const startTime = Math.round(Date.now() / 1000) - 86400;
            const endTime = Math.round(Date.now() / 1000) + 86400;

            const initData =
                SelfClaimERC20BudgetApproval.interface.encodeFunctionData(
                    'initialize',
                    getCreateSelfClaimErc20TokenBAParams({
        ...
        it('throws "DuplicatedToAddress"', async () => {
            const initData =
                SelfClaimERC20BudgetApproval.interface.encodeFunctionData(
                    'initialize',
                    getCreateSelfClaimErc20TokenBAParams({
    ...
    describe('Execute Transaction (Transfer ETH)', async () => {
        ...
        it('executes transfer ETH', async () => {
            const entity = receiver.address;
            const nonce = 1;
            const messageHash = await budgetApproval.getMessageHash(
                entity,
                nonce,
            );
        ...
        it('throws RecipientNotWhitelisted if Receiver not whitelisted', async () => {
            ...
            const transactionData = abiCoder.encode(
                await budgetApproval.executeParams(),
                [executor.address, nonce, signature],
            );
            const tx = await budgetApproval
                .connect(executor)
                .createTransaction(
        ...
        it('throws "AddressClaimed" if the accumulated amount > fix amount', async () => {
            const entity = receiver.address;
            const nonce = 3;
            const messageHash = await budgetApproval.getMessageHash(
                entity,
            ...
            const firstTx = await budgetApproval
                .connect(executor)
                .createTransaction(
                    [transactionData],
                    Math.round(Date.now() / 1000) + 86400,
            ...
            const { id: firstTxId } = await findEventArgs(
                firstTx,
                'CreateTransaction',
            );
            ...
            await budgetApproval
                .connect(approver)
                .approveTransaction(firstTxId, '');
            await expect(
                budgetApproval.connect(executor).executeTransaction(firstTxId),
            ).to.not.be.reverted;
        ...
        it('throws "SignatureNotCorrrect" if the entity not match', async () => {
            const entity = executor.address;
            const nonce = 4;
            const messageHash = await budgetApproval.getMessageHash(
                entity,
            ...
            await expect(
                ...
                SelfClaimERC20BudgetApproval,
                'SignatureNotCorrrect',
            );
        });

        it('throws "SignatureNotCorrrect" if the nonce not match', async () => {
            const entity = executor.address;
            const nonce = 5;
            const wrongNonce = 77;
            const messageHash = await budgetApproval.getMessageHash(
            ...
            const transactionData = abiCoder.encode(
                await budgetApproval.executeParams(),
                [receiver.address, wrongNonce, signature],
            );
            const tx = await budgetApproval
                .connect(executor)
                .createTransaction(
        ...
        it('throws "SignatureNotCorrrect" if the signature not signed correctly', async () => {
            const entity = executor.address;
            const nonce = 5;
            const messageHash = await budgetApproval.getMessageHash(
                entity,
            ...
            const transactionData = abiCoder.encode(
                await budgetApproval.executeParams(),
                [receiver.address, nonce, messageHash],
            );
            const tx = await budgetApproval
                .connect(executor)
                .createTransaction(
    ...
    describe('Execute Transaction (Transfer ERC20)', async () => {
        const fixAmount = ethers.utils.parseEther('10');
        beforeEach(async () => {
            await executor.sendTransaction({
                to: executee.address,
