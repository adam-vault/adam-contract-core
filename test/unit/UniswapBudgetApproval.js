...
describe('UniswapBudgetApproval.sol - test/unit/UniswapBudgetApproval.js', async function () {
  ...
  beforeEach(async function () {
    ...
    UniswapBudgetApproval = await ethers.getContractFactory('UniswapBudgetApproval', { signer: executeeAsSigner });
    ERC1967Proxy = await ethers.getContractFactory('ERC1967Proxy', { signer: executeeAsSigner });

    uniswapBAImpl = await UniswapBudgetApproval.deploy();
  });
  ...
  describe('executeParams()', async function () {
    ...
    beforeEach(async function () {
      const contract = await ERC1967Proxy.deploy(
        uniswapBAImpl.address,
        UniswapBudgetApproval.interface.encodeFunctionData('initialize', initializeParser()));
      uniswapBA = await ethers.getContractAt('UniswapBudgetApproval', contract.address);
    });
