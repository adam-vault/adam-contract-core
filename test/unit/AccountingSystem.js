...
describe('AccountingSystem.sol', async () => {
    let creator;
    let daoAsSigner;
    let dao;
    let priceGateway1;
    ...
    describe('initialize', async () => {
        it('should initialize the contract and set the dao and price gateways correctly', async () => {
            ...
            expect(await accountingSystem.defaultPriceGateway()).to.equal(
                priceGateway1.address,
            );
            expect(await accountingSystem.priceGateways(priceGateway1.address))
                .to.be.true;
        });
        it('should fail if the contract is already initialized', async () => {
            await expect(
                accountingSystem.initialize(dao.address, [
                    priceGateway1.address,
                ]),
            ).to.be.rejected;
        });
    ...
    describe('addPriceGateway', async () => {
        it('add a price gateway to the whitelist', async () => {
            ...
            await accountingSystem
                .connect(daoAsSigner)
                .addPriceGateway(priceGateway2.address);
            expect(await accountingSystem.priceGateways(priceGateway2.address))
                .to.be.true;
        });

        it('fail if the price gateway is already whitelisted', async () => {
            await expect(
                accountingSystem.addPriceGateway(priceGateway1.address),
            ).to.be.rejected;
        });

        it('should fail if the sender is not the govern', async () => {
            await expect(
                accountingSystem.addPriceGateway(priceGateway3.address),
            ).to.be.rejected;
        });
    });
    ...
    describe('setTokenPairPriceGatewayMap', async () => {
        it('should set the price gateway for the provided token pairs', async () => {
            priceGateway1.isSupportedPair
                .whenCalledWith(tokenA.address, tokenB.address)
                .returns(true);
            priceGateway1.isSupportedPair
                .whenCalledWith(tokenB.address, tokenA.address)
                .returns(true);
            await accountingSystem
                .connect(daoAsSigner)
            ...
            expect(
                await accountingSystem.tokenPairPriceGatewayMap(
                    tokenA.address,
                    tokenB.address,
                ),
            ).to.equal(priceGateway1.address);
            expect(
                await accountingSystem.tokenPairPriceGatewayMap(
                    tokenB.address,
                    tokenA.address,
        ...
        it('should fail if the sender is not the dao', async () => {
            ...
            await expect(
                accountingSystem.setTokenPairPriceGatewayMap(
                    [tokenA.address],
                    [tokenB.address],
                    priceGateway1.address,
                ),
    ...
    describe('assetPrice', async () => {
        it('should redirect the function call to price Gateway', async () => {
            ...
            await accountingSystem.assetPrice(
                tokenA.address,
                tokenB.address,
                parseEther('1'),
            );
            expect(priceGateway1.assetPrice).to.have.been.calledWith(
                tokenA.address,
                tokenB.address,
                parseEther('1'),
            );
            ...
            await accountingSystem.assetPrice(
                ...
                tokenA.address,
                parseEther('1'),
            ); // fallback to default pricegateway
            expect(priceGateway1.assetPrice).to.have.been.calledWith(
                tokenB.address,
                tokenA.address,
                parseEther('1'),
        ...
        it('reverts with "Not Supported Price Pair" when calls with not Supported token ', async () => {
            await expect(
                accountingSystem.assetPrice(
                    notSupportedToken.address,
                    ADDRESS_ETH,
                    parseEther('1'),
                ),
            ).to.be.revertedWithCustomError(accountingSystem, 'PairNotSupport');
        });
    });
    ...
    describe('isSupportedPair', async () => {
        it('should redirect the function call to price Gateway', async () => {
            ...
            expect(
                await accountingSystem.isSupportedPair(
                    tokenB.address,
                    tokenA.address,
                ),
            ).to.be.equal(true); // fallback to defaultPriceGateway
            ...
            expect(priceGateway1.isSupportedPair).to.have.been.calledWith(
                tokenA.address,
                tokenB.address,
            );
        });
