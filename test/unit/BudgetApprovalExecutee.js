...
describe('BudgetApprovalExecuteeV2.sol - test/unit/BudgetApprovalExecutee.js', async () => {
    let deployer;
    let team;
    let budgetApproval;
    let unknown;
    ...
    beforeEach(async () => {
        ...
        executee = await (
            await smock.mock('MockBudgetApprovalExecutee', deployer)
        ).deploy();
    ...
    describe('budgetApprovals()', async () => {
        it('returns if address in budgetApproval mapping', async () => {
            expect(
                await executee.budgetApprovals(budgetApproval.address),
            ).to.eq(true);
            expect(await executee.budgetApprovals(unknown.address)).to.eq(
                false,
            );
        });
    });
    describe('executeByBudgetApproval()', async () => {
        it('executes call and values to target contract', async () => {
            await executee
                .connect(budgetApproval)
    ...
    describe('createBudgetApprovals()', async () => {
        let mockBudgetApproval;
        let params;
        beforeEach(async () => {
            mockBudgetApproval = await (
                await smock.mock('MockBudgetApproval', deployer)
            ).deploy();
            params = [
                [
        ...
        it('executes call and values to target contract', async () => {
            const tx = await executee.createBudgetApprovals(
                ...
                [
                    mockBudgetApproval.interface.encodeFunctionData(
                        'initialize',
                        params,
                    ),
                ],
    ...
    describe('revokeBudgetApprovals()', async () => {
        it('revokes target budget approvals', async () => {
            const tx = await executee.revokeBudgetApprovals([
        ...
        it('throws "BudgetApprovalNotExists"', async () => {
            await expect(
                executee.revokeBudgetApprovals([unknown.address]),
            ).to.be.revertedWithCustomError(
                executee,
                'BudgetApprovalNotExists',
            );
        });
    });
});
