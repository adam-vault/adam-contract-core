...
import {UniswapBytesLib} from './UniswapBytesLib.sol';
import {UniswapConstants} from './UniswapConstants.sol';
...
library UniswapV3Path {
    ...
    function hasMultiplePools(bytes calldata path) internal pure returns (bool) {
        return path.length >= UniswapConstants.MULTIPLE_V3_POOLS_MIN_LENGTH;
    }
    ...
    function getFirstPool(bytes calldata path) internal pure returns (bytes calldata) {
        return path[:UniswapConstants.V3_POP_OFFSET];
    }
    ...
    function skipToken(bytes calldata path) internal pure returns (bytes calldata) {
        return path[UniswapConstants.NEXT_V3_POOL_OFFSET:];
    }
}
