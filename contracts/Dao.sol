    ...
    BudgetApprovalExecutee
{
    ...
    // V2
    address public override accountSystem;
        ...
        initializer
    {
        require(
            address(params._creator) != address(0) &&
                ...
                address(params._accountSystem) != address(0),
            "Invaild Dao Setting"
        );
        _initializing = true;

        ___BudgetApprovalExecutee_init(params._team, params._accountSystem);
        ...
        accountSystem = params._accountSystem;
        _mintMember(params._creator);
        _createMemberToken(
            params._memberTokenImplementation,
    ...
    function byPassGovern(address account) public view returns (bool) {
        return (IMembership(membership).totalSupply() == 1 &&
            isMember(account));
    }
    ...
    function transferMemberToken(address to, uint amount)
        public
        onlyGovern("General")
    {
    ...
    function setMinDepositAmount(uint256 _minDepositAmount)
        public
        onlyGovern("General")
    {
    ...
    function createGovern(
        ...
        if (voteType == VoteType.Membership) {
            ...
            require(
                _membership != address(0),
                "Membership not yet initialized"
            );
            _voteToken = _membership;
        } else if (voteType == VoteType.MemberToken) {
            address _memberToken = memberToken;
            require(
                _memberToken != address(0),
                "MemberToken not yet initialized"
            );
    ...
    function setMemberTokenAsAdmissionToken(uint256 minTokenToAdmit)
        public
        onlyGovern("General")
    {
        address _memberToken = memberToken;
        _addAdmissionToken(_memberToken, minTokenToAdmit, 0);
        emit AddAdmissionToken(_memberToken, minTokenToAdmit, 0, true);
    }
    ...
    function addAdmissionToken(
        ...
        emit AddAdmissionToken(token, minTokenToAdmit, tokenId, false);
    }
    ...
    function _createMemberToken(
        ...
        ERC1967Proxy _memberTokenContract = new ERC1967Proxy(
    ...
    function isPassAdmissionToken(address account) public view returns (bool) {
        ...
        for (uint i = 0; i < _admissionTokenLength; i++) {
            ...
            uint256 _minTokenToAdmit = admissionTokenSetting[token]
                .minTokenToAdmit;
            ...
            if (_minTokenToAdmit > 0) {
                ...
                if (token.isERC721()) {
                    ...
                    balance = IERC1155(token).balanceOf(
                        account,
                        admissionTokenSetting[token].tokenId
                    );
    ...
    function afterDeposit(address account, uint256 amount) external {
        ...
        if (firstDepositTime[account] == 0) {
            ...
            require(
                isPassAdmissionToken(account),
                "Admission token not enough"
            );
            _mintMember(account);
    ...
    function _beforeCreateBudgetApproval(address budgetApproval)
        internal
        view
        override
        onlyGovern("General")
    {
        require(
            canCreateBudgetApproval(budgetApproval),
            "Budget Implementation not whitelisted"
        );
    }

    function _beforeRevokeBudgetApproval(address budgetApproval)
        internal
        view
        override
        onlyGovern("General")
    ...
    function _addAdmissionToken(
        ...
        require(
            admissionTokens.length <= 3,
            "Admission Token length too long."
        );
    }
    ...
    function _addAssets(address[] memory erc20s) internal {
        for (uint256 i = 0; i < erc20s.length; i++) {
            require(
                address(erc20s[i]) != address(0),
                "Not Supported ERC20 Token"
            );
            _addAsset(erc20s[i]);
        }
    }
    ...
    function upgradeImplementations(
        ...
        require(
            targets.length == newImplementations.length,
            "params length not match"
        );
    ...
    function quit(uint256 membershipTokenId) public {
        ...
        require(
            ILiquidPool(payable(liquidPool)).balanceOf(_member) == 0,
            "LP balance is not zero"
        );
        require(
            IMembership(_membership).isMember(_member) == true,
            "Not a member"
        );
    ...
    function multicall(
        ...
        bytes[] calldata data
    ) public onlyGovern("General") returns (bytes[] memory) {
        require(
