...
async function main () {
  ...
  const { daoAddress } = await inquirer
    .prompt([
      { type: 'input', name: 'daoAddress', message: 'Dao?', choices: daos.map(({ dao, name }) => ({ name: `${dao} - ${name}`, value: dao })) },
    ]);
  ...
  const { contractFn, txValue } = await inquirer
    .prompt([
      { type: 'list', name: 'contractFn', message: 'Dao?', choices: fns },
      { type: 'input', name: 'txValue', message: 'How much ETH to pass?', default: '0' },
    ]);

  const params = await inquirer
    .prompt(contractFn.inputs.map((pType, i) => ({
      type: 'input',
      name: `${i}`,
  ...
  console.log('Going to run with', contractFn.key, 'with params', finalParams, 'with value', hre.ethers.utils.parseEther(txValue));

  const { shouldExecute } = await inquirer
    .prompt([
      { type: 'confirm', name: 'shouldExecute', message: 'Should Execute?'},
    ]);
  if (shouldExecute) {
    const tx = await dao[contractFn.key](...finalParams, { value: hre.ethers.utils.parseEther(txValue) });
    console.log(tx);
  } else {
    console.log(dao.interface.encodeFunctionData(contractFn.key, finalParams));
  }
}
