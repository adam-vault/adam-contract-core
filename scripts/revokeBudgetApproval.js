...
async function main() {
    ...
    const { daoAddress, dest } = await inquirer.prompt([
        {
            type: 'list',
            name: 'daoAddress',
            message: 'Dao?',
    ...
    if (dest === 'LiquidPool') {
        ...
        tx = await dao.revokeBudgetApprovals([budgetApproval.address]);
    }
    ...
    const RevokeEventLogs1 = _.filter(receipt1.events, {
        event: 'RevokeBudgetApproval',
    });
