...
async function generateInterface({
    ...
    return Promise.all(
        matchedFiles.map(async (filepath) => {
            ...
            const updatedAbi = JSON.parse(
                JSON.stringify(abi).replace(/struct /gi, `struct ${prefix}`),
            );
            ...
            await writeFile(
                path.join(__dirname, '..', outputPath, '/', `${name}.sol`),
                output,
            );
...
task('interface', 'Generate interface for contracts', async (args, hre) =>
    generateInterface({
        artifacts: './artifacts/contracts',
        contracts: 'contracts/*.sol',
        outputPath: './contracts/interface',
