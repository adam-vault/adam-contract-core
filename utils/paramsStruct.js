...
function getCreateTransferERC20BAParams({
    ...
    return Object.entries({
        ...
        toAddresses,
        allowAllTokens:
            allowAllTokens ?? token === ethers.constants.AddressZero,
        token,
    ...
    return [
        ...
        [
            iface.encodeFunctionData('createPlugin', [
                ...
                membershipIface.encodeFunctionData('initialize', [
                    name,
                    maxMemberLimit,
                ]),
            ]),
            ...
            minDepositAmount
                ? iface.encodeFunctionData('setMinDepositAmount', [
                      minDepositAmount,
                  ])
                : '',
            ...
            creator
                ? iface.encodeFunctionData('executePlugin', [
                      ethers.utils.id('adam.dao.membership'),
                      membershipIface.encodeFunctionData('createMember', [
                          creator,
                      ]),
                      0,
                  ])
            ...
            tokenAmount
                ? iface.encodeFunctionData('executePlugin', [
                      ethers.utils.id('adam.dao.member_token'),
                      memberTokenIface.encodeFunctionData('mintToOwner', [
                          tokenAmount,
                      ]),
                      0,
                  ])
                ...
                ([token, minTokenToAdmit, tokenId, isMemberToken]) => {
                    if (isMemberToken) {
                        return iface.encodeFunctionData('executePlugin', [
                            ethers.utils.id('adam.dao.membership'),
                            membershipIface.encodeFunctionData(
                                'setMemberTokenAsAdmissionToken',
                                [minTokenToAdmit],
                            ),
                            0,
                        ]);
